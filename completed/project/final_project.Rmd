...
```{r NA vals rate, echo=FALSE}
sapply(fertility__rate_df, function(x) sum(is.na(x)))
```
```{r NA vals pop, echo=FALSE}
...
```{r NA vals remove preg, echo=FALSE}
preg <- preg[ , colSums(is.na(preg)) == 0]
```

```{r NA vals resp, echo=FALSE}
...
```{r NA vals remove resp, echo=FALSE}
...
```{r merge rate and pop, echo=FALSE}
...
```{r merge preg and resp, echo=FALSE}
...
```{r final fertility, echo=FALSE}
head(fertility_df)
str(fertility_df)
```
```{r merged pop, echo=FALSE}
head(rate_pop_merged)
str(rate_pop_merged)
```
```{r merged nsfg, echo=FALSE}
